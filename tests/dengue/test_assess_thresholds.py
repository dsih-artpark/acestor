...
def test_prev_nweeks_selected_when_historical_has_no_rows():
    """prev_nweeks is selected when historical has Row_Count_Total=0."""
    rows = [
        _make_final_row(
            "2024-01-01", "historical", row_count_total=0, risk_zone_sum_total=0.0
        ),
        _make_final_row(
            "2024-01-01", "previousNweeks", row_count_total=3, risk_zone_sum_total=7.0
        ),
    ]
    final = pd.DataFrame(rows)
    best = _select_best_method(final)
...
def test_weighted_baseline_is_last_priority():
    """weighted_baseline is selected only when historical and prev_nweeks both have no rows."""
    rows = [
        _make_final_row(
            "2024-01-01", "historical", row_count_total=0, risk_zone_sum_total=0.0
        ),
        _make_final_row(
            "2024-01-01", "previousNweeks", row_count_total=0, risk_zone_sum_total=0.0
        ),
        _make_final_row(
            "2024-01-01", "weightedBaseline", row_count_total=4, risk_zone_sum_total=8.0
        ),
    ]
    final = pd.DataFrame(rows)
    best = _select_best_method(final)
...
def test_selection_per_date_independent():
    ...
    rows = [
        ...
        # date2: historical has no rows → pick prev_nweeks
        _make_final_row(
            "2024-01-08", "historical", row_count_total=0, risk_zone_sum_total=0.0
        ),
        _make_final_row(
            "2024-01-08", "previousNweeks", row_count_total=3, risk_zone_sum_total=7.0
        ),
    ]
    final = pd.DataFrame(rows)
    best = _select_best_method(final)
...
def test_all_methods_zero_rows_falls_back_to_any():
    """When all methods have Row_Count_Total=0, returns first available (no crash)."""
    rows = [
        _make_final_row(
            "2024-01-01", "historical", row_count_total=0, risk_zone_sum_total=0.0
        ),
