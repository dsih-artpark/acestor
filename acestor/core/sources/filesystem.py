...
from acestor.io import FileSource


class FileSystemSource:
    """Filesystem source with optional cache behavior.

    strategy:
    - local: read local cache only
    - cloud: read remote base only
    - local_first: try local cache, fallback remote
    - cloud_first: try remote, fallback local cache
    """
    ...
    def read(self, relpath: str) -> bytes:
        ...
        # cloud_first (or unknown) => try remote then fallback to local cache.
        try:
            data = self.remote.read(rel)
            self._write_cache(rel, data)
            return data
